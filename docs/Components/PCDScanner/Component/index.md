...
This component captures separate point clouds for **static** and **dynamic** points:

- **Static points** are candidates for RSU (Roadside Unit) placement.
- **Dynamic points** represent positions on the car where LiDAR sensors would be used for an autonomous vehicle.

Captured files are named according to the time step and saved accordingly. Compared to the default PCD component of AWSIM, which only supports a dynamic scanner with a single sensor and merges all files.
The path of the autonomous vehicle can be provided in two ways:

1. **CSV File**:  
   A CSV file containing scan positions is provided.

2. **Lanelet Map**:  
   A lanelet map is given, and the scans are generated along the path.

<div style="text-align: center;">
  <img src="image-4.png" alt="alt text" width="1200">
  <p> Sample scan captured by a static sensor</p>
</div>
